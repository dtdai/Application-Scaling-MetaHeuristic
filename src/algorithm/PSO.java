...
public class PSO {
    ...
    public PSO(int numApp, ArrayList<Machine> machine, int numParticle, int Iteration, double W, double C1, double C2) {
        ...
        appBundle = new AppBundle(numApp, pms, vms);
    }
...
//        }
    ...
    private void GeneratePosition(ParticleNode particle) {
        for (int p = 0; p < Particle; p++) {
            ...
            ArrayList<Integer> fullTour = new ArrayList<>();
            fullTour.addAll(appBundle.getTour());
            fullTour.addAll(tour);
            particle.pos.add(VirtualAllocation(particle, tour, fullTour));
        }
    }
    ...
    private double VirtualAllocation(ParticleNode particle, ArrayList<Integer> tour, ArrayList<Integer> fullTour) {
        ArrayList<PhysicalMachine> hosts = CloneHost();
        ...
        particle.model = new GameModel(appBundle.getApp(), hosts, vms, fullTour);

        return particle.model.BenefitFunction();
    }
    ...
    private void SufflePosition(ParticleNode particle, int index, int value) {
        for (int it = 0; it < value; it++) {
            ...
            while (newtour.size() != appBundle.getCount() - appBundle.getTour().size()) {
                newtour = GenerateTour();
            }
            
            fulltour.addAll(appBundle.getTour());
            fulltour.addAll(newtour);
            double newpos = VirtualAllocation(particle, newtour, fulltour);
